---
title: "Unity 性能分析工具完全手册：从入门到精通的调试宝典"
date: 2020/05/08
categories: [技术文章, Unity开发]
tags: [Unity, 性能分析, Profiler, 渲染优化, ARM, Xcode, Android Studio, RenderDoc, UWA, UPR]
image: /images/unity-profiling-tools-complete-guide-banner.jpg
---

# 🛠️ Unity 性能分析工具完全手册：从入门到精通的调试宝典

> 💡 **性能分析工具的价值**：
> - 游戏卡顿、发热、耗电快，如何找到问题根源？
> - Unity Profiler、ARM Profiler、RenderDoc，该用哪个？
> - 如何系统性地分析游戏性能问题？
> - 从 CPU 到 GPU，从内存到渲染，全工具有哪些？
>
> **完全手册**！涵盖 Unity 开发中最常用的性能分析工具，让性能优化有迹可循！

---

- [https://github.com/leinlin/Miku-LuaProfiler](https://github.com/leinlin/Miku-LuaProfiler)

Unity profiler & Unity Profile Analyzer

- Profile Analyzer这个工具可以解析 profller数据,只有300 帧的数据.在平常简单情况下可以快速识别耗时函数.

- 在一帧 16.66 ms,即 1 秒 60 帧的情况下,单个函数调用一般情况下要低于 1ms,所有渲染函数集合总耗时最好不要超过 5.5ms,即渲染函数总耗时可以占用一帧时间的 1/3.

- 同理,在一帧 33.33 ms,即 1秒 30 帧的情况下,单个函数调用一般情况下要低于 1ms,所有渲染函数集合总耗时最好不要超过 11ms,即渲染函数总耗时可以占用一帧时间的 1/3.

编译时间

- Unity Editor Iteration Profiler (EIP) 

- NeedleTools Compilation Visualizer and EIP,[https://github.com/openupm/openupm.git](https://github.com/openupm/openupm.git)

Frame DebugPhysics DebugSnapdragon profiler[https://developer.qualcomm.com/software/snapdragon-profiler](https://developer.qualcomm.com/software/snapdragon-profiler)

Adreno profiler[https://developer.qualcomm.com/download/adrenoprofiler/adreno-profiler_osx.dmg](https://developer.qualcomm.com/download/adrenoprofiler/adreno-profiler_osx.dmg)

System Analyzer[https://software.intel.com/content/www/us/en/develop/tools/graphics-performance-analyzers/system-analyzer.html](https://software.intel.com/content/www/us/en/develop/tools/graphics-performance-analyzers/system-analyzer.html)
